(function() {
  var CWD, VERSION, cli, compileJade, compileSource, compileStylus, createBlogPost, createFolders, createProject, exec, fs, jade, jottOptions, log, moment, path, projectDir, prompt, quitWithMsg, readSettings, stylus, util,
    _this = this;
...
include ../templates/footer\n';
      return fs.outputFile(projectDir + '/src/jade/posts/' + postTitle.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "-").replace(/^-+|-+$/g, '') + '.jade', string, function(err) {
        ...
        exec('echo "mixin link(\'' + postTitle + '\')"|cat - src/jade/items.jade  > /tmp/out && mv /tmp/out src/jade/items.jade', function(err, stdout, stderr) {
          err && (function() {
            throw err;
          })();
